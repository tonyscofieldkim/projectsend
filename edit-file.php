...
/**
								 * Edit a* Edit a file name or description.
					...
					if ($current_level != 0) {
						...
						if (!empty($file['assignments'])) {
							...
							foreach ($file_on_clients as $this_client) {
								$compare_clients[] = 'c' . $this_client;
							}
							foreach ($file_on_groups as $this_group) {
								$compare_groups[] = 'g' . $this_group;
							}
	...
	?>
		<form action="edit-file.php?file_id=<?php echo filter_var($this_file_id, FILTER_VALIDATE_INT); ?>" method="post" name="edit_file" id="edit_file">
			<div class="container-fluid">
				...
				?>
					<div class="file_editor <?php if ($i % 2) {
												echo 'f_e_odd';
											} ?>">
						<div class="row">
							<div class="col-sm-12">
											...
											<div class="row">
												<div class="col-sm-12">
													...
													<div class="form-group">
														...
														<textarea name="file[<?php echo $i; ?>][description]" class="<?php if (DESCRIPTIONS_USE_CKEDITOR == 1) {
																															echo 'ckeditor';
																														} ?> form-control" placeholder="<?php _e('Optionally, enter here a description for the file.', 'cftp_admin'); ?>"><?php echo (!empty($row['description'])) ? html_output($row['description']) : ''; ?></textarea>
													</div>
												</div>
											</div>
										</div>
											...
											<div class="file_data">
												...
												<select multiple="multiple" name="file[<?php echo $i; ?>][assignments][]" class="form-control chosen-select" data-placeholder="<?php _e('Select one or more options. Type to search.', 'cftp_admin'); ?>">
													<optgroup label="<?php _e('Clients', 'cftp_admin'); ?>">
														...
														?>
															<option value="<?php echo 'c' . $client; ?>" <?php if (in_array($client, $file_on_clients)) {
													...
													<optgroup label="<?php _e('Groups', 'cftp_admin'); ?>">
														...
														?>
															<option value="<?php echo 'g' . $group; ?>" <?php if (in_array($group, $file_on_groups)) {
																											echo ' selected="selected"';
																										} ?>>
																<?php echo $group_name; ?>
															</option>
